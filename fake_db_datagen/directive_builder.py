...
class GeneratorDirectiveDependency:
    ...
    def __init__(self
        ...
        , referenced_directive: 'GeneratorDirective'
    ) -> None:
...
class GeneratorDirective:
    ...
    def __init__(self
        , index: int
        , name: str
        , num_samples_per_ref: int
        , fields: Dict[str, DataTypeGenerator]
        , dependencies: Dict[str, GeneratorDirectiveDependency]
    ):
        self.index = index
        self.name = name
        self.fields = fields
    ...
    def __generate_dependencies_entries(self
    ) -> List[Dict[str, Any]]:

        # auxiliar functions

        clone_entries = lambda entries: [{k:v for k,v in e.items()} for e in entries]
        ...
        for field_name, dependency in self.dependencies.items():
            ...
            dependency_values = dependency.directive.fetch_generated(field=dependency.referenced_field)

            # calculate new dependency_entries

            dependency_values_entries = {field: value for value in dependency_values}
            ...
            if not dependency_entries:
                generated_dependency_entries = dependency_values_entries
            else:

                new_dependency_entries = []

                for dependency_entry in dependency_values_entries:
                    current_dependency_entry_entries = [{**dependency_entry, **entry} for entry in clone_entries(entries)]
    ...
    def __generate_value_entries(self
        , num_samples: int
    ) -> List[Dict[str, Any]]:
    ...
    def __cross_dependency_and_value_entries(self
        , dependencies_entries: List[Dict[str, Any]]
        , value_entries: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:

        # auxiliar functions

        chunk_list = lambda l, chunk_size: [l[i:i + chunk_size] for i in range(0, len(l), chunk_size)]
        ...
        chunked_value_entries = chunk_list(value_entries, self.num_samples)
    ...
    def _generate(self) -> List[Dict[str, Any]]:
        ...
        if self.dependencies:
            ...
            num_samples = self.num_samples * len(entries)
            ...
            logger.debug(f'crossing with dependency entries with generated entry values')

            entries = self.__cross_dependency_and_value_entries(
                dependencies_entries=dependencies_entries
        ...
        logger.debug(f'formatting generated dependencies.')

        self._generated_data = {
            field: [entry[field] for entry in entries]
            for field in entires[0].keys()
        }
    ...
    def reset(self) -> None:
        logger.debug(f'reset  {self.num_samples} samples for field {self.table}.{field}')
...
class DirectiveBuilder:
    ...
    def _calculate_dependencies(self
        ...
        for table in dbml.tables:
            ...
            table_refs = {ref.current_table_field: {
                ...
                , 'field': ref.referenced_table_field
    ...
    def __build_directive_for_table(self
        , table: str
        , table_config: Dict[str, Dict[str, Any]]
        , table_dependencices: Dict[str, Dict[str, Dict[str, str]]]
    ) -> GeneratorDirective:
        """
        ...
        for field, field_configuration in table_config.items():
            ...
            data_generator = DataTypeGenerator.from_type(
                ...
                , start=field_configuration.get('start')
                , end=field_configuration.get('end')
                , generable_expression=field_configuration.get('generable_expression')
                , collection_values=field_configuration.get('collection_values')
                , distribution_generator=field_configuration.get('distribution_generator')
            )
    ...
    def __build_dependencies_for_table_directive(self
        , table: str
        , directives: List[GeneratorDirective]
        , table_dependencices: Dict[str, Dict[str, Dict[str, str]]]
    ) -> None:
            ...
            for field, dependency in table_dependencices.items():
                ...
                dependency_directive = GeneratorDirectiveDependency(
                    ...
                    , referenced_directive=referenced_directive
                )
    ...
    def _build_directives(self
        ...
        for table, table_configuration in tables_config.items():
            ...
            directive = self.__build_directive_for_table(
                ...
                , table_dependencices=table_dependencices
            )
    ...
    def build(self
        ...
        logger.debug(f'calculating dependencies')
        dependencies = self._calculate_dependencies(
            dbml=dbml
        )

        # create directives mixing the populate and the dependencies (to avoid multiple data generation)
        logger.debug(f'instancing building directives')
        ...
        logger.debug(f'calculating table sequency')
