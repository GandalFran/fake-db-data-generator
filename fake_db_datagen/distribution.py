...
class DistributionGenerator:
    ...
    def scale(self
        ...
        origin_start = np.min(values)
        origin_end = np.max(values)
...
class NormalDistributionGenerator:
    ...
    def generate(self
        ...
        # generate
        values = np.random.normal(size=num_sampes)

        # scale
        scaled_values = self.scale(
            origin_values=values
            , dest_start=min_value
            , dest_end=max_value
        )
...
class LogDistributionGenerator:
    ...
    def generate(self
        ...
        # generate
        values = np.random.lognormal(size=num_sampes)
